...
export const redrawTextBoundingBox = (
  ...
  if (container) {
    ...
    const isFallbackPadding =
      isArrowElement(container) ||
      !(container as ExcalidrawTextContainer).containerPadding;
    const [padX, padY] = getContainerPadding(
...
export const getContainerCoords = (container: NonDeletedExcalidrawElement) => {
  const isFallbackPadding =
    isArrowElement(container) ||
    !(container as ExcalidrawTextContainer).containerPadding;
  let [offsetX, offsetY] = getContainerPadding(
    container as ExcalidrawTextContainer,
    isFallbackPadding,
  );
...
export const computeContainerDimensionForBoundText = (
  ...
  if (containerType === "arrow") {
    return dimension + BOUND_TEXT_PADDING * 8; // should padding change for arrow?
  }
...
export const getBoundTextMaxHeight = (
  ...
  if (isArrowElement(container)) {
    const containerHeight = height - offsetY * 8 * 2; // keep the same?
...
export const getContainerPadding = (
  ...
  return [CONTAINER_PADDING_X, CONTAINER_PADDING_Y];
};
